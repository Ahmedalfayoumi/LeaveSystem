...
export const useAppData = () => {
    ...
    const addNotification = useCallback(async (message: string, type: NotificationType) => {
    ...
    const onImportFromCode = useCallback(async (code: string) => {
        if (!currentUser) return;
    ...
    const handleAddMultipleEmployees = useCallback(async (employeesData: Omit<Employee, 'id'>[]) => {
    ...
    const handleDeleteEmployees = useCallback(async (ids: string[]) => {
        if (!currentUser) return;
    ...
    const handleAddBalanceAdjustment = useCallback(async (adjustment: BalanceAdjustment) => {
        if (!currentUser) return;
    ...
    const handleAddLeave = useCallback(async (leaveData: Omit<Leave, 'id'>): Promise<Leave> => {
    ...
    const handleUpdateLeave = useCallback(async (leave: Leave) => {
        if (!currentUser) return;
    ...
    }, [currentUser, leaves, employees, addNotification]);

    const handleAddDeparture = useCallback(async (dep: Omit<Departure, 'id'>): Promise<Departure> => {
        if (!currentUser) return Promise.reject('No current user');
    ...
    const handleUpdateDeparture = useCallback(async (dep: Departure) => {
        if (!currentUser) return;
    ...
    const handleDeleteDeparture = useCallback(async (id: string) => {
        if (!currentUser) return;
    ...
    }, [currentUser, departures, employees, addNotification]);

    const handleAddHoliday = useCallback(async (hol: Omit<Holiday, 'id'>) => {
        if (!currentUser) return;
    ...
    }, [currentUser, holidays, addNotification]);

    const handleAddHolidayWork = useCallback(async (hw: Omit<HolidayWork, 'id'>) => {
        if (!currentUser) return;
    ...
    const handleDeleteHolidayWork = useCallback(async (id: string) => {
        if (!currentUser) return;
    ...
    }, [currentUser, holidayWork, employees, addNotification]);

    const handleUpdateCompanyInfo = useCallback(async (info: CompanyInfo) => {
        if (!currentUser) return;
    ...
    const handleUpdateNationalities = useCallback(async (items: string[]) => {
        if (!currentUser) return;
    ...
    const handleUpdateIdTypes = useCallback(async (items: string[]) => {
        if (!currentUser) return;
